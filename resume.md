...
清华大学电子工程系博士，研究方向为计算机视觉与生物识别；具备生成式图像复原、计算摄影、RAW 域画质优化与端侧算法落地经验。现任荣耀高级算法工程师，长期负责手机影像核心算法研发，覆盖算法设计、数据闭环、跨部门协同与量产交付。
...
负责手机影像相关算法预研与落地，重点方向包括文字场景图像复原、异构 CFA 复原算法、RAW 域联合去噪去马赛克(JDD)算法交付与多机型画质优化。
...
- 画质问题攻坚： 针对暗区涂抹、伪纹理、色阶断层等高频 PQ 问题，引入亮度自适应退化策略进行有效优化，完成问题闭环。

## &#xecfa; 专业技能

- 算法方向：计算机视觉、图像复原、生成式视觉算法、计算摄影、RAW 域画质优化、生物识别。
- 研发方法：使用 Hydra / OmegaConf 进行实验配置管理，结合 Aim 等工具构建可追踪、可复现的算法研发流程。
- AI 提效/辅助开发：在实际业务中深度集成 Claude Code / Codex / Antigravity 等 AI 工具，并能够基于 Agentic Tools 其设计思想合理设计 CLAUDE.md/AGENTS.md 进行项目管理与 Skills 提效。
...
发表与参与论文 7 篇，其中一作 3 篇，研究方向聚焦生物识别与图像复原。
